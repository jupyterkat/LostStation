...
/obj/machinery/computer/apc_control/Topic(href, href_list)
	...
	if(href_list["above_filter"])
		playsound(src, 'sound/machines/terminal_prompt.ogg', 50, 0)
		var/new_filter = input(usr, "Enter a percentage from 1-100 to sort by (greater than).", name)
		if(!src || !usr || !usr.canUseTopic(src) || stat || QDELETED(src))
			return
	...
	if(href_list["below_filter"])
		playsound(src, 'sound/machines/terminal_prompt.ogg', 50, 0)
		var/new_filter = input(usr, "Enter a percentage from 1-100 to sort by (lesser than).", name)
		if(!src || !usr || !usr.canUseTopic(src) || stat || QDELETED(src))
			return
